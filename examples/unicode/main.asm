...
restart:
  ...
  0x255a		0f8ec3000000		JLE 0x2623			
			ps.i -= 1
  ...
  0x259f		660f1f840000000000	NOPW 0(AX)(AX*1)		
  0x25a8		0f1f4000		NOPL 0(AX)			
  0x25ac		4181fae381a7e3		CMPL $-0x1c587e1d, R10		
  0x25b3		0f853fffffff		JNE 0x24f8			
  0x25b9		470fb7540804		MOVZX 0x4(R8)(R9*1), R10	
  0x25bf		664181fa8199		CMPW $-0x667f, R10		
  0x25c5		0f852dffffff		JNE 0x24f8			
	c[1] = i // end of match
  0x25cb		4889742478		MOVQ SI, 0x78(SP)	
		if matched {
  0x25d0		84d2			TESTL DL, DL		
	if !matched || c[1]-c[0] > bc[1]-bc[0] {
  0x25d2		744a			JE 0x261e		
  0x25d4		4c8b4c2470		MOVQ 0x70(SP), R9	
  0x25d9		4989f2			MOVQ SI, R10		
  0x25dc		4c29ce			SUBQ R9, SI		
  0x25df		4c8b8c2498000000	MOVQ 0x98(SP), R9	
  0x25e7		4c2b8c2490000000	SUBQ 0x90(SP), R9	
  0x25ef		4c39ce			CMPQ R9, SI		
  0x25f2		7e22			JLE 0x2616		
		bc = c
  0x25f4		0f10442470		MOVUPS 0x70(SP), X0	
  0x25f9		0f11842490000000	MOVUPS X0, 0x90(SP)	
  0x2601		0f10842480000000	MOVUPS 0x80(SP), X0	
  0x2609		0f118424a0000000	MOVUPS X0, 0xa0(SP)	
  0x2611		ba01000000		MOVL $0x1, DX		
		if i <= len(r) && len(bt) > 0 {
  0x2616		4c89d6			MOVQ R10, SI		
	goto fail
  0x2619		e9ddfeffff		JMP 0x24fb		
		if i <= len(r) && len(bt) > 0 {
  0x261e		4989f2			MOVQ SI, R10		
	if !matched || c[1]-c[0] > bc[1]-bc[0] {
  0x2621		ebd1			JMP 0x25f4		
	switch bt[len(bt)-1].pc {
  0x2623		48ffc9			DECQ CX			
			bt = bt[:n]
  0x2626		e93fffffff		JMP 0x256a		
  0x262b		90			NOPL			
		if matched {
...
TEXT type..eq.[2]string(SB) gofile..<autogenerated>
  ...
  0x3da0		64488b0c2500000000	MOVQ FS:0, CX			[5:9]R_TLS_LE		
  0x3da9		483b6110		CMPQ 0x10(CX), SP		
  0x3dad		0f86a6000000		JBE 0x3e59			
  0x3db3		4883ec30		SUBQ $0x30, SP			
  0x3db7		48896c2428		MOVQ BP, 0x28(SP)		
  0x3dbc		488d6c2428		LEAQ 0x28(SP), BP		
  0x3dc1		488b442438		MOVQ 0x38(SP), AX		
  0x3dc6		488b4808		MOVQ 0x8(AX), CX		
  0x3dca		488b542440		MOVQ 0x40(SP), DX		
  0x3dcf		48394a08		CMPQ CX, 0x8(DX)		
  0x3dd3		7575			JNE 0x3e4a			
  0x3dd5		488b4a18		MOVQ 0x18(DX), CX		
  0x3dd9		48394818		CMPQ CX, 0x18(AX)		
  0x3ddd		756b			JNE 0x3e4a			
  0x3ddf		31c9			XORL CX, CX			
  0x3de1		eb13			JMP 0x3df6			
  0x3de3		488b5c2420		MOVQ 0x20(SP), BX		
  0x3de8		488d4b01		LEAQ 0x1(BX), CX		
  0x3dec		488b442438		MOVQ 0x38(SP), AX		
  0x3df1		488b542440		MOVQ 0x40(SP), DX		
  0x3df6		4883f902		CMPQ $0x2, CX			
  0x3dfa		7d3f			JGE 0x3e3b			
  0x3dfc		48894c2420		MOVQ CX, 0x20(SP)		
  0x3e01		48c1e104		SHLQ $0x4, CX			
  0x3e05		488b3411		MOVQ 0(CX)(DX*1), SI		
  0x3e09		488b3c01		MOVQ 0(CX)(AX*1), DI		
  0x3e0d		488b4c0808		MOVQ 0x8(AX)(CX*1), CX		
  0x3e12		48893c24		MOVQ DI, 0(SP)			
  0x3e16		4889742408		MOVQ SI, 0x8(SP)		
  0x3e1b		48894c2410		MOVQ CX, 0x10(SP)		
  0x3e20		e800000000		CALL 0x3e25			[1:5]R_CALL:runtime.memequal	
  0x3e25		807c241800		CMPB $0x0, 0x18(SP)		
  0x3e2a		75b7			JNE 0x3de3			
  0x3e2c		c644244800		MOVB $0x0, 0x48(SP)		
  0x3e31		488b6c2428		MOVQ 0x28(SP), BP		
  0x3e36		4883c430		ADDQ $0x30, SP			
  0x3e3a		c3			RET				
  0x3e3b		c644244801		MOVB $0x1, 0x48(SP)		
  0x3e40		488b6c2428		MOVQ 0x28(SP), BP		
  0x3e45		4883c430		ADDQ $0x30, SP			
  0x3e49		c3			RET				
  0x3e4a		c644244800		MOVB $0x0, 0x48(SP)		
  0x3e4f		488b6c2428		MOVQ 0x28(SP), BP		
  0x3e54		4883c430		ADDQ $0x30, SP			
  0x3e58		c3			RET				
  0x3e59		e800000000		CALL 0x3e5e			[1:5]R_CALL:runtime.morestack_noctxt	
  0x3e5e		6690			NOPW				
  0x3e60		e93bffffff		JMP type..eq.[2]string(SB)	
