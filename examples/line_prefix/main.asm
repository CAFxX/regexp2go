...
restart:
  ...
  0x2702		e90a020000		JMP 0x2911		
		if matched {
  0x2707		4189c0			MOVL AX, R8		
		if i <= len(r) && len(bt) > 0 {
  0x270a		4c89d8			MOVQ R11, AX		
  0x270d		4839f0			CMPQ SI, AX		
  0x2710		0f8fa2010000		JG 0x28b8		
  0x2716		4885ff			TESTQ DI, DI		
  0x2719		0f8e99010000		JLE 0x28b8		
	switch bt[len(bt)-1].pc {
  0x271f		4c6bcf38		IMULQ $0x38, DI, R9		
  0x2723		4d8b5411f0		MOVQ -0x10(R9)(DX*1), R10	
  0x2728		0f1f00			NOPL 0(AX)			
	case 5:
  ...
  0x280a		b801000000		MOVL $0x1, AX		
		matched = true
  0x280f		e9f3feffff		JMP 0x2707		
		if matched {
  0x2814		4489c0			MOVL R8, AX		
	goto fail
  0x2817		e9ebfeffff		JMP 0x2707		
		if i <= len(r) && len(bt) > 0 {
  0x281c		4d89d3			MOVQ R10, R11		
	if !matched || c[1]-c[0] > bc[1]-bc[0] {
  0x281f		ebc9			JMP 0x27ea		
		if after == '\n' || after == -1 {
  0x2821		4183f9ff		CMPL $-0x1, R9		
  0x2825		7493			JE 0x27ba		
		if i <= len(r) && len(bt) > 0 {
  0x2827		4c89d0			MOVQ R10, AX		
  0x282a		90			NOPL			
		goto fail
  0x282b		e9ddfeffff		JMP 0x270d		
		if i <= len(r) && len(bt) > 0 {
...
TEXT type..eq.[2]string(SB) gofile..<autogenerated>
  ...
  0x4130		64488b0c2500000000	MOVQ FS:0, CX			[5:9]R_TLS_LE		
  0x4139		483b6110		CMPQ 0x10(CX), SP		
  0x413d		0f86a6000000		JBE 0x41e9			
  0x4143		4883ec30		SUBQ $0x30, SP			
  0x4147		48896c2428		MOVQ BP, 0x28(SP)		
  0x414c		488d6c2428		LEAQ 0x28(SP), BP		
  0x4151		488b442438		MOVQ 0x38(SP), AX		
  0x4156		488b4808		MOVQ 0x8(AX), CX		
  0x415a		488b542440		MOVQ 0x40(SP), DX		
  0x415f		48394a08		CMPQ CX, 0x8(DX)		
  0x4163		7575			JNE 0x41da			
  0x4165		488b4a18		MOVQ 0x18(DX), CX		
  0x4169		48394818		CMPQ CX, 0x18(AX)		
  0x416d		756b			JNE 0x41da			
  0x416f		31c9			XORL CX, CX			
  0x4171		eb13			JMP 0x4186			
  0x4173		488b5c2420		MOVQ 0x20(SP), BX		
  0x4178		488d4b01		LEAQ 0x1(BX), CX		
  0x417c		488b442438		MOVQ 0x38(SP), AX		
  0x4181		488b542440		MOVQ 0x40(SP), DX		
  0x4186		4883f902		CMPQ $0x2, CX			
  0x418a		7d3f			JGE 0x41cb			
  0x418c		48894c2420		MOVQ CX, 0x20(SP)		
  0x4191		48c1e104		SHLQ $0x4, CX			
  0x4195		488b3411		MOVQ 0(CX)(DX*1), SI		
  0x4199		488b3c01		MOVQ 0(CX)(AX*1), DI		
  0x419d		488b4c0808		MOVQ 0x8(AX)(CX*1), CX		
  0x41a2		48893c24		MOVQ DI, 0(SP)			
  0x41a6		4889742408		MOVQ SI, 0x8(SP)		
  0x41ab		48894c2410		MOVQ CX, 0x10(SP)		
  0x41b0		e800000000		CALL 0x41b5			[1:5]R_CALL:runtime.memequal	
  0x41b5		807c241800		CMPB $0x0, 0x18(SP)		
  0x41ba		75b7			JNE 0x4173			
  0x41bc		c644244800		MOVB $0x0, 0x48(SP)		
  0x41c1		488b6c2428		MOVQ 0x28(SP), BP		
  0x41c6		4883c430		ADDQ $0x30, SP			
  0x41ca		c3			RET				
  0x41cb		c644244801		MOVB $0x1, 0x48(SP)		
  0x41d0		488b6c2428		MOVQ 0x28(SP), BP		
  0x41d5		4883c430		ADDQ $0x30, SP			
  0x41d9		c3			RET				
  0x41da		c644244800		MOVB $0x0, 0x48(SP)		
  0x41df		488b6c2428		MOVQ 0x28(SP), BP		
  0x41e4		4883c430		ADDQ $0x30, SP			
  0x41e8		c3			RET				
  0x41e9		e800000000		CALL 0x41ee			[1:5]R_CALL:runtime.morestack_noctxt	
  0x41ee		6690			NOPW				
  0x41f0		e93bffffff		JMP type..eq.[2]string(SB)	
