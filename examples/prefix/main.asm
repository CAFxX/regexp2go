...
TEXT type..eq.[2]string(SB) gofile..<autogenerated>
  ...
  0x3a14		64488b0c2500000000	MOVQ FS:0, CX			[5:9]R_TLS_LE		
  0x3a1d		483b6110		CMPQ 0x10(CX), SP		
  0x3a21		0f86a6000000		JBE 0x3acd			
  0x3a27		4883ec30		SUBQ $0x30, SP			
  0x3a2b		48896c2428		MOVQ BP, 0x28(SP)		
  0x3a30		488d6c2428		LEAQ 0x28(SP), BP		
  0x3a35		488b442438		MOVQ 0x38(SP), AX		
  0x3a3a		488b4808		MOVQ 0x8(AX), CX		
  0x3a3e		488b542440		MOVQ 0x40(SP), DX		
  0x3a43		48394a08		CMPQ CX, 0x8(DX)		
  0x3a47		7575			JNE 0x3abe			
  0x3a49		488b4a18		MOVQ 0x18(DX), CX		
  0x3a4d		48394818		CMPQ CX, 0x18(AX)		
  0x3a51		756b			JNE 0x3abe			
  0x3a53		31c9			XORL CX, CX			
  0x3a55		eb13			JMP 0x3a6a			
  0x3a57		488b5c2420		MOVQ 0x20(SP), BX		
  0x3a5c		488d4b01		LEAQ 0x1(BX), CX		
  0x3a60		488b442438		MOVQ 0x38(SP), AX		
  0x3a65		488b542440		MOVQ 0x40(SP), DX		
  0x3a6a		4883f902		CMPQ $0x2, CX			
  0x3a6e		7d3f			JGE 0x3aaf			
  0x3a70		48894c2420		MOVQ CX, 0x20(SP)		
  0x3a75		48c1e104		SHLQ $0x4, CX			
  0x3a79		488b3411		MOVQ 0(CX)(DX*1), SI		
  0x3a7d		488b3c01		MOVQ 0(CX)(AX*1), DI		
  0x3a81		488b4c0808		MOVQ 0x8(AX)(CX*1), CX		
  0x3a86		48893c24		MOVQ DI, 0(SP)			
  0x3a8a		4889742408		MOVQ SI, 0x8(SP)		
  0x3a8f		48894c2410		MOVQ CX, 0x10(SP)		
  0x3a94		e800000000		CALL 0x3a99			[1:5]R_CALL:runtime.memequal	
  0x3a99		807c241800		CMPB $0x0, 0x18(SP)		
  0x3a9e		75b7			JNE 0x3a57			
  0x3aa0		c644244800		MOVB $0x0, 0x48(SP)		
  0x3aa5		488b6c2428		MOVQ 0x28(SP), BP		
  0x3aaa		4883c430		ADDQ $0x30, SP			
  0x3aae		c3			RET				
  0x3aaf		c644244801		MOVB $0x1, 0x48(SP)		
  0x3ab4		488b6c2428		MOVQ 0x28(SP), BP		
  0x3ab9		4883c430		ADDQ $0x30, SP			
  0x3abd		c3			RET				
  0x3abe		c644244800		MOVB $0x0, 0x48(SP)		
  0x3ac3		488b6c2428		MOVQ 0x28(SP), BP		
  0x3ac8		4883c430		ADDQ $0x30, SP			
  0x3acc		c3			RET				
  0x3acd		e800000000		CALL 0x3ad2			[1:5]R_CALL:runtime.morestack_noctxt	
  0x3ad2		6690			NOPW				
  0x3ad4		e93bffffff		JMP type..eq.[2]string(SB)	
