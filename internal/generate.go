...
func Generate(regex, pkg, fn string, flags uint, usePool bool) ([]byte, error) {
	...
	// TODO: at every step where this can be determined, instead of just checking if we are at the end of the string check if we are too far from
	//       and/or too close to the end of the string, to be able to skip visiting states if there is no chance that we will end in a terminal state
	...
	if pkg == "main" {
		out(`
			...
			import "os"
			func main() {
				m, _, found := %s{}.FindLongestString(os.Args[1])
				if !found {
					os.Exit(-1)
				}
				for i, c := range m {
	...
	out(`
		...
		const (
			modeMatch%[1]s modeType%[1]s = iota
			modeFirst%[1]s
			modeLongest%[1]s
		)
	`, fn)
	...
	out("  matches, pos, ok = e.doString(r, modeFirst%[1]s, bt[:0])", fn)
	out("  return")
	out("}")
	out("")
	...
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = e.doString(r, modeLongest%[1]s, bt[:0])", fn)
	out("  return")
	out("}")
	out("")
	...
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = e.doByteSlice(s, modeFirst%[1]s, bt[:0])", fn)
	out("  return")
	out("}")
	out("")
	...
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = e.doByteSlice(s, modeLongest%[1]s, bt[:0])", fn)
	out("  return")
	out("}")
	out("")
	...
	out(`
		...
		func (e %[1]s) doString(s string, m modeType%[1]s, bt []state%[1]s) (matches [%[2]d]string, pos int, ok bool) {
			var pmatches [%[2]d*2]int
	...
	out("func (e %[1]s) do(r string, m modeType%[1]s, bt []state%[1]s) ([%[2]d]int, bool) {", fn, p.NumCap)
	...
	out(`
		...
		match:
			if !matched || c[1] - c[0] > bc[1] - bc[0] {
				if m == modeMatch%[1]s || m == modeFirst%[1]s {
					return c, true
				}
				bc = c
				matched = true
