...
func Generate(regex, pkg, fn string, flags uint, usePool bool) ([]byte, error) {
	...
	// TODO: switch to type methods instead of standalone functions
	...
	if pkg == "main" {
		out(`
			...
			import "os"
			func main() {
				m, _, found := %sLongestString(os.Args[1])
				if !found {
					os.Exit(-1)
				}
				for i, c := range m {
	...
	out("func %sString(r string) (matches [%d]string, pos int, ok bool) {", fn, p.NumCap/2)
	...
	out("  matches, pos, ok = doString%[1]s(r, %[1]sMatchFirst, bt[:0])", fn)
	out("  return")
	out("}")
	out("")

	out("// %sLongestString implements the regular expression\n// %v\n// with flags %d and returning the leftmost-longest match.", fn, regex, flags)
	out("func %sLongestString(r string) (matches [%d]string, pos int, ok bool) {", fn, p.NumCap/2)
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = doString%[1]s(r, %[1]sMatchLongest, bt[:0])", fn)
	out("  return")
	out("}")
	out("")

	out("// %s implements the regular expression\n// %v\n// with flags %d and returning the first leftmost match.", fn, regex, flags)
	out("func %s(s []byte) (matches [%d][]byte, pos int, ok bool) {", fn, p.NumCap/2)
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = doByteSlice%[1]s(s, %[1]sMatchFirst, bt[:0])", fn)
	out("  return")
	out("}")
	out("")

	out("// %sLongest implements the regular expression\n// %v\n// with flags %d and returning the leftmost-longest match.", fn, regex, flags)
	out("func %sLongest(s []byte) (matches [%d][]byte, pos int, ok bool) {", fn, p.NumCap/2)
	out("  var bt [%d]state%s // static storage for backtracking state", numSt, fn)
	out("  matches, pos, ok = doByteSlice%[1]s(s, %[1]sMatchLongest, bt[:0])", fn)
	out("  return")
	out("}")
	out("")
	...
	out(`
		func doByteSlice%[1]s(s []byte, m %[1]sMode, bt []state%[1]s) (matches [%[2]d][]byte, pos int, ok bool) {
			var r string
